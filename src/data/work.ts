...
/** Bounds of a period string: "2026 \u2014 now", "2025 \u2014 26" or a bare "2024". */
const parsePeriod = (period: string): { start: number; end: number | "now" } => {
  const [from, to] = period.split("\u2014").map((part) => part.trim());
  const start = Number(from);
  if (!to) return { start, end: start };
  if (to === "now") return { start, end: "now" };
  return { start, end: Number(to.length === 2 ? `20${to}` : to) };
};

const formatSpan = (start: number, last: number | "now"): string => {
  if (last === "now") return `${start} \u2014 now`;
  if (last === start) return `${start}`;
  return `${start} \u2014 ${String(last).slice(2)}`;
};

export interface ClientTenure {
  /** Span across every engagement with this client, e.g. "2020 \u2014 24". */
  period: string;
  /** How many separate engagements ran with them. */
  count: number;
}

/**
 * Years and engagement count per client, folded out of `engagements` so the
 * home page never carries a second, drifting copy of the same dates.
 */
export const clientTenures: Record<string, ClientTenure> = (() => {
  const spans = new Map<string, { start: number; last: number | "now"; count: number }>();

  for (const { company, period } of engagements) {
    const { start, end } = parsePeriod(period);
    const seen = spans.get(company);
    spans.set(company, {
      start: Math.min(start, seen?.start ?? start),
      last:
        seen?.last === "now" || end === "now"
          ? "now"
          : Math.max(end, seen?.last ?? end),
      count: (seen?.count ?? 0) + 1,
    });
  }

  return Object.fromEntries(
    [...spans].map(([company, span]) => [
      company,
      { period: formatSpan(span.start, span.last), count: span.count },
    ]),
  );
})();

/** Total engagements listed on /work. */
export const engagementCount = engagements.length;

/** Earliest year any engagement started. */
export const firstEngagementYear = Math.min(
  ...engagements.map((engagement) => parsePeriod(engagement.period).start),
);
